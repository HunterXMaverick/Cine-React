...
class Rooms extends Component {
    handleOpenModal (id) { 
        this.setState({ showModal: true, test: id }) 
    }
    handleCloseModal () { this.setState({ showModal: false }) }

    handleOpenModal2 (id) { 
        this.setState({ showModal1: true, test: id }) 
    }
    handleCloseModal2 () { this.setState({ showModal1: false }) }
    ...
    upData(e) {
        ...
        axios.put(`http://localhost:5000/film/sala/?id=${this.state.test}`, {datos})
                ...
                <div className="ml-64">
                    ...
                    <main className="my-8">
                        <div className="justify-center my-5 select-none flex">
                            ...
                            <ReactModal id="editar" isOpen={this.state.showModal1} contentLabel="onRequestClose Example" onRequestClose={this.handleCloseModal1}
                                className="flex-1 text-white text-center pl-48 py  py-0 my-10 mr-40 ml-64">
                                <div className="leading-loose">
                                    <form className="max-w-xl m-4 p-10 bg-white rounded shadow-xl" onSubmit={ this.upData.bind(this) }>
                                        <p className="text-gray-800 font-medium">Editar Sala</p>
                                            <div className="mt-2">
                                                <label className="block text-sm text-blue-800" htmlFor="nombre">Nombre</label>
                                            ...
                                            <div className="mt-4">
                                                <button className="px-4 py-1 text-white font-light tracking-wider bg-gray-900 hover:bg-gray-800 rounded" type="submit">Editar</button>
                                            </div>
                                    </form>
                            ...
                            <table className="w-full text-md bg-white shadow-md rounded mb-4">
                                ...
                                <tbody>
                                    <tr className="border-b hover:bg-green-200 bg-gray-100">
                                        ...
                                        <td>
                                            { salas.map(element => <p className="p-2 px-5" key={ element.id }><button onClick={ () => this.handleOpenModal2(element.id) } className="mr-3 text-sm bg-blue-900 hover:bg-blue-900 text-white py-1 px-2 rounded focus:outline-none focus:shadow-outline">Editar</button></p> )}
                                        </td>
                                        <td>
